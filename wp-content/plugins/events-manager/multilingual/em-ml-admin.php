...
class EM_ML_Admin{
	...
	public static function meta_boxes(){
	    ...
	    if( !empty($EM_Event) && !EM_ML::is_original($EM_Event) ){
		    ...
		    if( get_option('dbem_attributes_enabled', true) ){
	    	    ...
	    	    add_meta_box('em-event-attributes', __('Attributes','dbem'), 'EM_ML_Admin::event_meta_box_attributes',EM_POST_TYPE_EVENT, 'normal','default');
	    	}
		    //add translation-specific meta boxes
		    add_meta_box('em-event-translation', __('Translated Event Information','dbem'), 'EM_ML_Admin::meta_box_translated_event',EM_POST_TYPE_EVENT, 'side','high');
		    $event = EM_ML::get_original_event($EM_Event);
		    if( $event->event_rsvp ){
		        add_meta_box('em-event-bookings-translation', __('Bookings/Registration','dbem'), 'EM_ML_Admin::meta_box_bookings_translation',EM_POST_TYPE_EVENT, 'normal','high');
		    }
	    }
	    ...
	    if( !empty($EM_Location) && !EM_ML::is_original($EM_Location) ){
	        //add translation-specific meta boxes
		    add_meta_box('em-event-translation', __('Translated Event Information','dbem'), 'EM_ML_Admin::meta_box_translated_location',EM_POST_TYPE_LOCATION, 'side','high');
	    	...
	    	if( get_option('dbem_location_attributes_enabled') ){
	    	    ...
	    	    add_meta_box('em-location-attributes', __('Attributes','dbem'), 'EM_ML_Admin::location_meta_box_attributes',EM_POST_TYPE_LOCATION, 'normal','default');
	    	}
	    }
	}
	...
	public static function meta_box_translated_event(){
	    ...
	    <p>
			...
			echo __('This is a translated event, therefore your time, location and booking information is handled by your original event translation.', 'dbem');
			echo ' <a href="'.esc_url($original_link).'">'.__('See original translation.','dbem').'</a>';
	    	?>
	    </p>
	    <?php
	}
	...
	public static function meta_box_translated_location(){
	    ...
	    <p>
			...
			echo __('This is a translated event, therefore address information is handled by the original location translation.', 'dbem');
			echo ' <a href="'.esc_url($original_link).'">'.__('See original translation.','dbem').'</a>';
	    	?>
	    </p>
	    <?php
