...
<div class="em-ticket-form">
	...
	<div class="em-ticket-form-main">
		<div class="ticket-name">
			<label title="<?php esc_attr_e('Enter a ticket name.','dbem'); ?>"><?php esc_html_e('Name','dbem') ?></label>
		...
		<div class="ticket-description">
			<label><?php esc_html_e('Description','dbem') ?></label>
		...
		<div class="ticket-price"><label><?php esc_html_e('Price','dbem') ?></label><input type="text" name="em_tickets[<?php echo $col_count; ?>][ticket_price]" class="ticket_price" value="<?php echo esc_attr($EM_Ticket->get_price_precise()) ?>" /></div>
		<div class="ticket-spaces">
			<label title="<?php esc_attr_e('Enter a maximum number of spaces (required).','dbem'); ?>"><?php esc_html_e('Spaces','dbem') ?></label>
	...
	<div class="em-ticket-form-advanced" style="display:none;">
		<div class="ticket-spaces ticket-spaces-min">
			<label title="<?php esc_attr_e('Leave either blank for no upper/lower limit.','dbem'); ?>"><?php echo esc_html_x('At least','spaces per booking','dbem');?></label>
			...
			<?php esc_html_e('spaces per booking', 'dbem')?>
		</div>
		<div class="ticket-spaces ticket-spaces-max">
			<label title="<?php esc_attr_e('Leave either blank for no upper/lower limit.','dbem'); ?>"><?php echo esc_html_x('At most','spaces per booking', 'dbem'); ?></label>
		...
		<div class="ticket-dates em-time-range em-date-range">
			<div class="ticket-dates-from">
				<label title="<?php esc_attr_e('Add a start or end date (or both) to impose time constraints on ticket availability. Leave either blank for no upper/lower limit.','dbem'); ?>">
					<?php esc_html_e('Available from','dbem') ?>
				</label>
				...
				<div class="ticket-dates-from-recurring">
					...
					<?php esc_html_e('day(s)','dbem'); ?>
					<select name="em_tickets[<?php echo $col_count; ?>][ticket_start_recurring_when]" class="ticket-dates-from-recurring-when">
						<option value="before" <?php if( !empty($EM_Ticket->ticket_meta['recurrences']['start_days']) && $EM_Ticket->ticket_meta['recurrences']['start_days'] <= 0) echo 'selected="selected"'; ?>><?php echo esc_html(sprintf(_x('%s the event starts','before or after','dbem'),__('Before','dbem'))); ?></option>
						<option value="after" <?php if( !empty($EM_Ticket->ticket_meta['recurrences']['start_days']) && $EM_Ticket->ticket_meta['recurrences']['start_days'] > 0) echo 'selected="selected"'; ?>><?php echo esc_html(sprintf(_x('%s the event starts','before or after','dbem'),__('After','dbem'))); ?></option>
					</select>
				</div>
				<?php echo esc_html_x('at', 'time','dbem'); ?>
			...
			<div class="ticket-dates-to">
				<label title="<?php esc_attr_e('Add a start or end date (or both) to impose time constraints on ticket availability. Leave either blank for no upper/lower limit.','dbem'); ?>">
					<?php esc_html_e('Available until','dbem') ?>
				</label>
				...
				<div class="ticket-dates-to-recurring">
					...
					<select name="em_tickets[<?php echo $col_count; ?>][ticket_end_recurring_when]" class="ticket-dates-to-recurring-when">
						<option value="before" <?php if( !empty($EM_Ticket->ticket_meta['recurrences']['end_days']) && $EM_Ticket->ticket_meta['recurrences']['end_days'] <= 0) echo 'selected="selected"'; ?>><?php echo esc_html(sprintf(_x('%s the event starts','before or after','dbem'),__('Before','dbem'))); ?></option>
						<option value="after" <?php if( !empty($EM_Ticket->ticket_meta['recurrences']['end_days']) && $EM_Ticket->ticket_meta['recurrences']['end_days'] > 0) echo 'selected="selected"'; ?>><?php echo esc_html(sprintf(_x('%s the event starts','before or after','dbem'),__('After','dbem'))); ?></option>
					</select>
				</div>
				<?php echo esc_html_x('at', 'time','dbem'); ?>
		...
		<div class="ticket-required">
			<label title="<?php esc_attr_e('If checked every booking must select one or the minimum number of this ticket.','dbem'); ?>"><?php esc_html_e('Required?','dbem') ?></label>
		...
		<div class="ticket-type">
			<label><?php esc_html_e('Available for','dbem') ?></label>
			<select name="em_tickets[<?php echo $col_count; ?>][ticket_type]" class="ticket_type">
				<option value=""><?php _e('Everyone','dbem'); ?></option>
				<option value="members" <?php if($EM_Ticket->ticket_members) echo 'selected="selected"'; ?>><?php esc_html_e('Logged In Users','dbem'); ?></option>
				<option value="guests" <?php if($EM_Ticket->ticket_guests) echo 'selected="selected"'; ?>><?php esc_html_e('Guest Users','dbem'); ?></option>
			</select>
		</div>
	...
	<div class="ticket-options">
		<a href="#" class="ticket-options-advanced show"><span class="show-advanced"><?php esc_html_e('Show Advanced Options','dbem'); ?></span><span class="hide-advanced" style="display:none;"><?php esc_html_e('Hide Advanced Options','dbem'); ?></span></a>
	</div>
</div>	