...
class PreventPostDeletion_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'prevent-post-deletion'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'prevent-post-deletion'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'prevent-post-deletion'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'prevent-post-deletion'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'prevent-post-deletion');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'prevent-post-deletion'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'prevent-post-deletion');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'prevent-post-deletion'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'prevent-post-deletion') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'prevent-post-deletion');
            case 'false':
                return __('false', 'prevent-post-deletion');

            case 'Administrator':
                return __('Administrator', 'prevent-post-deletion');
            case 'Editor':
                return __('Editor', 'prevent-post-deletion');
            case 'Author':
                return __('Author', 'prevent-post-deletion');
            case 'Contributor':
                return __('Contributor', 'prevent-post-deletion');
            case 'Subscriber':
                return __('Subscriber', 'prevent-post-deletion');
            case 'Anyone':
                return __('Anyone', 'prevent-post-deletion');
        }
        return $optionValue;
    }
