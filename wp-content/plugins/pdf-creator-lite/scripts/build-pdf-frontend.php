...
*	.
*/
function SSAPDF_frontendBuildPDF ( $newIDarray, $titlepage, $toc, $filename )
{

	$hasFrontPage = ( $titlepage !== '' ) ? true : false;
	$hasToC = ( $toc === 'true' ) ? true : false;
	
	$displayTitle = ( isset($titlepage) && $titlepage !== '') ? $titlepage : get_the_title($newIDarray[0]);
	...
	*/
	if ( ! empty( $_POST['font'] ) ) {
		$text_font = $_POST['font'];
	}
	if ( ! empty( $_POST['fontcolor'] ) ) {
		$text_hex = $_POST['fontcolor'];
	}
	if ( ! empty( $_POST['linkcolor'] ) ) {
		$link_hex = $_POST['linkcolor'];
	}
	if ( ! empty( $_POST['bgcolor'] ) ) {
		$bg_rgb = SSAPDF_hex2RGB( $_POST['bgcolor'] );
	}
	$text_rgb = SSAPDF_hex2RGB( $text_hex );
	$link_rgb = SSAPDF_hex2RGB( $link_hex );
	...
	//{ 
		...
		*/
		...
		foreach ($newIDarray as $myID)
		{
			//$title = $page_data->post_title; 
			//$content = $page_data->post_content;
			$title = get_the_title( $myID );
			$content = SSAPDF_getPageContent($myID);
			...
			$depth = SSAPDF_getPageDepth( $myID );
			...
			if ( $title != 'Contents' )
			{
				...
				$pdf->AddPage();				
				$pdf->Bookmark( html_entity_decode($title, ENT_COMPAT, 'UTF-8'), $depth, 0, '', 'B', array($link_rgb['red'], $link_rgb['green'], $link_rgb['blue']), 0, '#TOC' );
